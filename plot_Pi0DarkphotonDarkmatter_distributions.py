"""Two-step decay: pi0 -> gamma + A' -> chi + chibar.
    ...
    python plot_Pi0DarkphotonDarkmatter_distributions.py \
        --m 3e-3 --alpha_D 0.5 --eps 1.6e-5 --n 1000
...
def generate_rows(
    ...
    primary process (step1) pi0 -> gamma + A',
    then propagates A' to a physically-sampled displaced vertex 
    via SecondaryPhysicalVertexDistribution before 
    secondary process(step2) sampling A' -> chi + chibar
    """
    ...
    def stopping_condition(datum, i):
        # Return False (don't stop) only for A' so SIREN processes A'->chi+chibar.
        # Default C++ stopping_condition always returns True (stop everything),
        return datum.record.signature.secondary_types[i] != dark_photon_type
    ...
    injector = siren.injection.Injector(
        ...
        secondary_interactions={step1.dark_photon_type: [step2]},
    ...
    for i in range(n_events):
        ...
        rows.append({
            ...
            "chi_cos_pi0": cos_angle(chi_p3, pi0_p4[1:4]),
            # chibar
            "chibar_E": chibar_E,
            ...
            "chibar_cos_pi0": cos_angle(chibar_p3, pi0_p4[1:4]),
            # chi+chibar pair
...
_CSV_KEYS = [
    ...
    "chi_kinetic_E", "chi_cos_beam", "chi_cos_aprime", "chi_cos_pi0",
    "chibar_E", "chibar_px", "chibar_py", "chibar_pz", "chibar_p",
    "chibar_kinetic_E", "chibar_cos_beam", "chibar_cos_aprime", "chibar_cos_pi0",
...
def main() -> None:
    ...
    aprime_plots = {
        ...
        "aprime_cos_beam.png": (
            "Dark photon cos(theta) wrt beam", "cos(theta_z)", column(rows, "aprime_cos_beam")
        ),
        "aprime_cos_pi0.png": (
            "Dark photon cos(theta) wrt pi0 (lab)", "cos(theta_{A', pi0})", column(rows, "aprime_cos_pi0")
        ),
        "aprime_beta.png": (
    ...
    two_panel_plot(
        ...
        "Dark matter cos(theta) wrt beam", "cos(theta_z) chi", "cos(theta_z) chibar",
    )
    two_panel_plot(
        column(rows, "chi_cos_aprime"), column(rows, "chibar_cos_aprime"),
        args.outdir / "dm_cos_aprime.png",
        "Dark matter cos(theta) wrt A' (lab)", "cos(theta_{chi, A'})", "cos(theta_{chibar, A'})",
    )
    two_panel_plot(
        column(rows, "chi_cos_pi0"), column(rows, "chibar_cos_pi0"),
        args.outdir / "dm_cos_pi0.png",
        "Dark matter cos(theta) wrt pi0 (lab)", "cos(theta_{chi, pi0})", "cos(theta_{chibar, pi0})",
    )

    # ---- chi+chibar pair ----
    inv_mass_residual = [value - args.m_med for value in column(rows, "chi_chibar_inv_mass")]
    histogram_plot(
        inv_mass_residual,
        args.outdir / "chi_chibar_inv_mass.png",
        f"Reconstructed chi+chibar invariant mass residual (m_A'={args.m_med:.4g} GeV)",
        "m_{chi chibar} - m_{A'} [GeV]",
    )
    histogram_plot(
