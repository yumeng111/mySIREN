...
"""Two-step cascade decay: pi0 -> gamma + A' -> gamma + chi + chibar.
...
def default_outdir(m_med: float, alpha_D: float) -> Path:
    return Path(f"outputs/pi0_cascade_med{m_med:g}_alphaD{alpha_D:g}")
...
def sample_decay_length(p_lab: float, mass: float, gamma_width: float, rand) -> float:
    """Draw lab-frame decay length [m] from exponential distribution."""
    if gamma_width <= 0.0 or mass <= 0.0 or p_lab < 0.0:
        return float("inf")
    mean_length = (p_lab / mass) * (HBAR_C_GEV_M / gamma_width)
    u = float(rand.Uniform(0.0, 1.0))
    return -mean_length * math.log(max(u, 1e-300))
...
def generate_rows(
    ...
    interactions = siren.interactions.InteractionCollection(pt.Pi0, [step1])
    sig1 = step1.GetPossibleSignatures()[0]   # pi0 -> gamma + A'
    sig2 = step2.GetPossibleSignatures()[0]   # A' -> chi + chibar
    ...
    for i in range(n_events):
        ...
        record1 = siren.dataclasses.InteractionRecord()
        primary_record.finalize(record1)
        record1.signature = sig1

        decay_record1 = siren.dataclasses.CrossSectionDistributionRecord(record1)
        step1.SampleFinalState(decay_record1, rand)
        decay_record1.finalize(record1)

        pi0_p4 = list(record1.primary_momentum)
        pi0_vertex = list(record1.interaction_vertex)
        aprime_p4 = list(record1.secondary_momenta[aprime_idx_1])
        gamma_p4 = list(record1.secondary_momenta[gamma_idx_1])

        #  Displaced A' decay vertex 
        aprime_p3 = aprime_p4[1:4]
        aprime_p = p_abs(*aprime_p3)
        decay_length = sample_decay_length(aprime_p, step2.m_dark_photon, gamma_width_aprime, rand)
        if math.isfinite(decay_length) and aprime_p > 0.0:
            n_hat = [aprime_p3[j] / aprime_p for j in range(3)]
            aprime_vertex = [pi0_vertex[j] + decay_length * n_hat[j] for j in range(3)]
        else:
            aprime_vertex = list(pi0_vertex)

        #  Step 2: A' -> chi + chibar at displaced vertex 
        record2 = siren.dataclasses.InteractionRecord()
        record2.signature = sig2
        record2.primary_momentum = aprime_p4
        record2.primary_mass = step2.m_dark_photon
        record2.interaction_vertex = aprime_vertex

        decay_record2 = siren.dataclasses.CrossSectionDistributionRecord(record2)
        step2.SampleFinalState(decay_record2, rand)
        decay_record2.finalize(record2)

        chi_p4 = list(record2.secondary_momenta[chi_idx_2])
        chibar_p4 = list(record2.secondary_momenta[chibar_idx_2])
...
_CSV_KEYS = [
    ...
    "chibar_kinetic_E", "chibar_cos_beam",
...
def main() -> None:
    ...
    two_panel_plot(
        column(rows, "chi_cos_aprime"), column(rows, "chi_cos_aprime"),
    ...
    gamma_width_aprime = step2._partial_width()
    ctau_aprime_m = HBAR_C_GEV_M / gamma_width_aprime if gamma_width_aprime > 0.0 else float("inf")
